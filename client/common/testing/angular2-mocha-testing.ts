...
export class TestInjector {
    public platformProviders: Array<Type | Provider | any[]> = [];

    public applicationProviders: Array<Type | Provider | any[]> = [];
    ...
    public addProviders(providers: Array<Type | Provider | any[]>) {
    ...
    public execute(fn: FunctionWithParamTokens): any {
        if (!this._instantiated) {
            this.createInjector();
        }
...
function runInTestZone(fnToExecute, finishCallback, failCallback): any {
    ...
    let pendingTimeouts = [];
    let ngTestZone = (<Zone>global['zone'])
        .fork({
            onError: function (e) {
                failCallback(e);
            },
...
export function beforeEach(fn: FunctionWithParamTokens | AnyTestFn): void {
    if (fn instanceof FunctionWithParamTokens) {
        ...
        jsmBeforeEach((done) => {
            ...
            let returnedTestValue =
                runInTestZone(() => testInjector.execute(fn), finishCallback, window['chai'].expect.fail);
            if (fn.isAsync) {
                if (_isPromiseLike(returnedTestValue)) {
                    (<Promise<any>>returnedTestValue).then(null, (err) => {
                        window['chai'].expect.fail(err);
                    });
                } else {
                    window['chai'].expect.fail('Error: injectAsync was expected to return a promise, but the ' +
                        ' returned value was: ' + returnedTestValue);
                }
            } else {
                if (!(returnedTestValue === undefined)) {
                    window['chai'].expect.fail('Error: inject returned a value. Did you mean to use injectAsync? Returned ' +
                        'value was: ' + returnedTestValue);
                }
            }
        });
...
export function setBaseTestProviders(platformProviders: Array<Type | Provider | any[]>,
    ...
    let useTestInjector = getTestInjector();
    if (useTestInjector.platformProviders.length > 0 || useTestInjector.applicationProviders.length > 0) {
        throw new BaseException('Cannot set base providers because it has already been called');
    }
    useTestInjector.platformProviders = platformProviders;
    useTestInjector.applicationProviders = applicationProviders;
    let injector = useTestInjector.createInjector();
    let inits: Function[] = injector.getOptional(PLATFORM_INITIALIZER);
    if (isPresent(inits)) {
        inits.forEach(init => init());
    }
    useTestInjector.reset();
}

export function resetBaseTestProviders() {
    let useTestInjector = getTestInjector();
    useTestInjector.platformProviders = [];
    useTestInjector.applicationProviders = [];
    useTestInjector.reset();
}
...
export class FunctionWithParamTokens {
    ...
    public execute(injector: Injector): any {
        let params = this._tokens.map(t => injector.get(t));
        return FunctionWrapper.apply(this._fn, params);
    }

    public hasToken(token: any): boolean {
